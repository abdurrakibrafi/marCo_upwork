...
class Command(BaseCommand):
    """Management command to scrape and seed handball club player rosters from Wikipedia squad tables."""
    help = "Scrape and seed handball club player rosters from Wikipedia"
    ...
    def handle(self, *args, **options):
        """Execute handball squad scraping from Wikipedia."""
        dry_run = options['dry_run']
        if dry_run:
            self.stdout.write(self.style.WARNING("=== DRY RUN MODE: Database changes will not be saved ==="))

        teams = Entity.objects.filter(type='team', sport='handball')
        self.stdout.write(f"Found {teams.count()} handball teams in the database.")

        if teams.count() == 0:
            self.stdout.write(self.style.WARNING("No handball teams found in database. Seeding will be skipped."))
            return
        ...
        for team in teams:
            ...
            search_query = f"{team.name} handball"
            ...
            for table in tables:
                ...
                if "player" in headers_text or "name" in headers_text:
                    if "pos" in headers_text or "position" in headers_text or "nat" in headers_text:
                        squad_table = table
                        break
            ...
            for r in rows:
                ...
                # We dynamically map indices based on table headers
                th_elements = r.find_parent('table').find_all('tr')[0].find_all(['th', 'td'])
                headers_list = [th.get_text().strip().lower() for th in th_elements]

                # If the first row headers are empty, try the second row (common in nested tables)
                ...
                # Default fallback indices if headers map is not clear
                player_idx = 1
                pos_idx = -1
                num_idx = 0
                nat_idx = -1
                ...
                # Ensure player_idx is valid
                if player_idx >= len(tds):
                    # Fallback to second td if out of range
                ...
                # Clean name (remove any parentheses/diacritics or details like (c))
                ...
                try:
                    ...
                    ae, created = Entity.objects.update_or_create(
                        ...
                        external_id=f"handball_wp_{team.id}_{self.normalize_string(fullname)}",
                        defaults={
                            'type': 'athlete',
                            'name': fullname,
                            'sport': 'handball',
        ...
        if not dry_run:
            self.stdout.write(self.style.SUCCESS(f"\nHandball backfill completed! Created {created_count} athletes, updated {updated_count} athletes."))
        else:
            self.stdout.write(self.style.SUCCESS("\nHandball backfill dry-run completed successfully!"))
