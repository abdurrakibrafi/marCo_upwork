...
from .converters import (
    _normalize_list,
    _clean_cricket_league_name,
    _convert_statpal_stats_to_api_sports,
    _convert_tennis_stats,
    _convert_generic_team_stats,
    _convert_statpal_events_to_api_sports,
    _make_absolute,
)
...
def get_live_score_detail_data(score_id, request=None):
    ...
    except (LiveScore.DoesNotExist, ValueError, TypeError):
        pass

    # 2. Try looking up Event ID -> check if live score exists for its external_id
    from apps.event.models import Event
    event_obj = Event.objects.filter(id=score_id).first() if not game else None
    ...
    # 4. If still not found anywhere, return None
    if not game:
        return None

    try:
        raw = getattr(game, 'raw_data', {})
        ...
        if sport == 'cricket':
            ...
            status_info = getattr(game, 'status_detail', None) or getattr(game, 'status', '')
            ...
            if sport == 'tennis':
                ...
                # Parse inning-by-inning scorecard for Baseball
        ...
        home_logo_val = _make_absolute(home_logo_val, request=request)
        ...
        away_logo_val = _make_absolute(away_logo_val, request=request)

        # Format start_time to ISO format if datetime object
        start_time_raw = getattr(game, 'start_time', None)
        if hasattr(start_time_raw, 'isoformat'):
            start_time_val = start_time_raw.isoformat()
        else:
        ...
        logger.error(f"Error in get_live_score_detail_data for game {getattr(game, 'id', None)}: {exc}", exc_info=True)
        from apps.entity.utils.matcher import find_team_logo_by_name
        home_logo_val = getattr(game, 'home_logo', '')
        is_invalid_home = home_logo_val and "statpal.io" in home_logo_val
        if (not home_logo_val or is_invalid_home) and getattr(game, 'home_team', None):
        ...
        fb_start_time = getattr(game, 'start_time', None)
        if hasattr(fb_start_time, 'isoformat'):
            fb_start_time = fb_start_time.isoformat()
